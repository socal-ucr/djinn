...
void caffe_gpu_gemm<float>(const CBLAS_TRANSPOSE TransA,
  ...
  CUBLAS_CHECK(cublasSgemm(cuTransB, cuTransA,
...
void caffe_gpu_gemm<double>(const CBLAS_TRANSPOSE TransA,
  ...
  cublasOperation_t cuTransB =
      (TransB == CblasNoTrans) ? CUBLAS_OP_N : CUBLAS_OP_T;
  CUBLAS_CHECK(cublasDgemm(cuTransB, cuTransA,
      N, M, K, &alpha, B, ldb, A, lda, &beta, C, N));
}
...
void caffe_gpu_gemv<float>(const CBLAS_TRANSPOSE TransA, const int M,
  ...
  CUBLAS_CHECK(cublasSgemv(cuTransA, N, M, &alpha,
      A, N, x, 1, &beta, y, 1));
}
...
void caffe_gpu_gemv<double>(const CBLAS_TRANSPOSE TransA, const int M,
  ...
  cublasOperation_t cuTransA =
      (TransA == CblasNoTrans) ? CUBLAS_OP_T : CUBLAS_OP_N;
  CUBLAS_CHECK(cublasDgemv(cuTransA, N, M, &alpha,
      A, N, x, 1, &beta, y, 1));
}
...
void caffe_gpu_axpy<float>(const int N, const float alpha, const float* X,
    float* Y) {
  CUBLAS_CHECK(cublasSaxpy(N, &alpha, X, 1, Y, 1));
}

template <>
void caffe_gpu_axpy<double>(const int N, const double alpha, const double* X,
    double* Y) {
  CUBLAS_CHECK(cublasDaxpy(N, &alpha, X, 1, Y, 1));
}
...
void caffe_gpu_scal<float>(const int N, const float alpha, float *X) {
  CUBLAS_CHECK(cublasSscal(N, &alpha, X, 1));
}

template <>
void caffe_gpu_scal<double>(const int N, const double alpha, double *X) {
  CUBLAS_CHECK(cublasDscal(N, &alpha, X, 1));
}
...
void caffe_gpu_dot<float>(const int n, const float* x, const float* y,
    float* out) {
  CUBLAS_CHECK(cublasSdot(n, x, 1, y, 1, out));
}

template <>
void caffe_gpu_dot<double>(const int n, const double* x, const double* y,
    double * out) {
  CUBLAS_CHECK(cublasDdot(n, x, 1, y, 1, out));
}

template <>
void caffe_gpu_asum<float>(const int n, const float* x, float* y) {
  CUBLAS_CHECK(cublasSasum(n, x, 1, y));
}

template <>
void caffe_gpu_asum<double>(const int n, const double* x, double* y) {
  CUBLAS_CHECK(cublasDasum(n, x, 1, y));
}

template <>
void caffe_gpu_scale<float>(const int n, const float alpha, const float *x,
                            float* y) {
  CUBLAS_CHECK(cublasScopy(n, x, 1, y, 1));
  CUBLAS_CHECK(cublasSscal(n, &alpha, y, 1));
}

template <>
void caffe_gpu_scale<double>(const int n, const double alpha, const double *x,
                             double* y) {
  CUBLAS_CHECK(cublasDcopy(n, x, 1, y, 1));
  CUBLAS_CHECK(cublasDscal(n, &alpha, y, 1));
}
