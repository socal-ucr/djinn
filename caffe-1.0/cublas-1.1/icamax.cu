...
/* This file contains the implementation of the BLAS-1 function icamax */
...
__global__ void icamax_gld_main (struct cublasIcamaxParams parms);
__global__ void icamax_tex_main (struct cublasIcamaxParams parms);
 ...
 */
__host__ int CUBLASAPI cublasIcamax (int n, const cuComplex *x, int incx)
{
    struct cublasContext *ctx = CUBLAS_GET_CTX();
    struct cublasIcamaxParams params;
    float *devPtrTmx;
    int *devPtrTix;
    cublasStatus status;
    ...
    if (n < CUBLAS_ICAMAX_CTAS) {
         nbrCtas = n;
         threadsPerCta = CUBLAS_ICAMAX_THREAD_COUNT;
    } else {
         nbrCtas = CUBLAS_ICAMAX_CTAS;
         threadsPerCta = CUBLAS_ICAMAX_THREAD_COUNT;
    }
    ...
    if (useTexture) {
        icamax_tex_main<<<nbrCtas,threadsPerCta>>>(params);
    } else {
        icamax_gld_main<<<nbrCtas,threadsPerCta>>>(params);
    }        
    ...
    for (i = 0; i < nbrCtas; i++) {
        xabs = tmx[i];
        jmax = tix[i];
        if ((xabs > cmax) || ((xabs == cmax) && (jmax < idx))) {
            idx = jmax;
            cmax = xabs;
        }
    }
...
__shared__ int   partialIMax[CUBLAS_ICAMAX_THREAD_COUNT];
__shared__ float partialCMax[CUBLAS_ICAMAX_THREAD_COUNT];
