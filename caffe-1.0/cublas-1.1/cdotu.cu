...
/* This file contains the implementation of the BLAS-1 function cdotu */
...
__global__ void cdotu_gld_main (struct cublasCdotuParams parms);
__global__ void cdotu_tex_main (struct cublasCdotuParams parms);

/*
 * cuComplex 
 * cublasCdotu (int n, const cuComplex *x, int incx, const cuComplex *y, 
 *              int incy)
 *
 ...
 */
__host__ cuComplex CUBLASAPI cublasCdotu (int n, const cuComplex *x, int incx, 
                                          const cuComplex *y, int incy)
{
    struct cublasContext *ctx = CUBLAS_GET_CTX();
    struct cublasCdotuParams params;
    ...
    if (n < CUBLAS_CDOTU_CTAS) {
         nbrCtas = n;
         threadsPerCta = CUBLAS_CDOTU_THREAD_COUNT;
    } else {
         nbrCtas = CUBLAS_CDOTU_CTAS;
         threadsPerCta = CUBLAS_CDOTU_THREAD_COUNT;
    }
    ...
    if (useTexture) {
        cdotu_tex_main<<<nbrCtas,threadsPerCta>>>(params);
    } else {
        cdotu_gld_main<<<nbrCtas,threadsPerCta>>>(params);
    }
