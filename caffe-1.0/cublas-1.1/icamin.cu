...
/* This file contains the implementation of the BLAS-1 function icamin */
...
__global__ void icamin_gld_main (struct cublasIcaminParams parms);
__global__ void icamin_tex_main (struct cublasIcaminParams parms);
 ...
 *
 * Reference: Analogous to ICAMAX, see there.
 *
 ...
 */
__host__ int CUBLASAPI cublasIcamin (int n, const cuComplex *x, int incx)
{
    struct cublasContext *ctx = CUBLAS_GET_CTX();
    struct cublasIcaminParams params;
    float *devPtrTmx;
    int *devPtrTix;
    cublasStatus status;
    ...
    if (n < CUBLAS_ICAMIN_CTAS) {
         nbrCtas = n;
         threadsPerCta = CUBLAS_ICAMIN_THREAD_COUNT;
    } else {
         nbrCtas = CUBLAS_ICAMIN_CTAS;
         threadsPerCta = CUBLAS_ICAMIN_THREAD_COUNT;
    }
    ...
    if (useTexture) {
        icamin_tex_main<<<nbrCtas,threadsPerCta>>>(params);
    } else {
        icamin_gld_main<<<nbrCtas,threadsPerCta>>>(params);
    }
    ...
    status = cublasGetVector (nbrCtas, sizeof(tmx[0]), devPtrTix, 1, tix, 1);
    if (status != CUBLAS_STATUS_SUCCESS) {
        cublasSetError (ctx, CUBLAS_STATUS_INTERNAL_ERROR);
        cublasFree (devPtrTmx);
        cublasFree (devPtrTix);
    ...
    for (i = 0; i < nbrCtas; i++) {
        xabs = tmx[i];
        jmin = tix[i];
        if ((xabs < cmin) || ((xabs == cmin) && (jmin < idx))) {
            idx = jmin;
            cmin = xabs;
        }
    }
...
__shared__ int   partialIMin[CUBLAS_ICAMIN_THREAD_COUNT];
__shared__ float partialCMin[CUBLAS_ICAMIN_THREAD_COUNT];
