...
__global__ void saxpy_gld_main (struct cublasSaxpyParams parms);
__global__ void saxpy_tex_main (struct cublasSaxpyParams parms);
 ...
 */
__host__ void CUBLASAPI cublasSaxpy (int n, float alpha, const float *x,
                                     int incx, float *y, int incy)
{
    struct cublasContext *ctx = CUBLAS_GET_CTX();
    struct cublasSaxpyParams params;
    ...
    cublasVectorSplay (n, CUBLAS_SAXPY_THREAD_MIN, CUBLAS_SAXPY_THREAD_MAX,
                       CUBLAS_SAXPY_CTAS_MAX, &nbrCtas, &elemsPerCta, 
                       &threadsPerCta);

    cudaStat = cudaGetLastError(); /* clear error status */
    if (useTexture) {
        saxpy_tex_main<<<nbrCtas,threadsPerCta>>>(params);
    } else {
        saxpy_gld_main<<<nbrCtas,threadsPerCta>>>(params);
    }
