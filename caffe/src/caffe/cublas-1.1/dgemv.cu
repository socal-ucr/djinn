...
/* This file contains the implementation of the BLAS-2 function dgemv */
...
__global__ void dgemvt_main (struct cublasDgemvParams parms);
__global__ void dgemvn_main (struct cublasDgemvParams parms);

/*
 * cublasDgemv (char trans, int m, int n, float alpha, const float *A, 
 ...
 */
__host__ void CUBLASAPI cublasDgemv (char trans, int m, int n, double alpha, 
                                     const double *A, int lda, const double *x, 
                                     int incx, double beta, double *y, int incy)
{
    struct cublasContext *ctx = CUBLAS_GET_CTX();
    struct cublasDgemvParams params;
    ...
    if (transpose) {
        dgemvt_main<<<CUBLAS_DGEMVT_CTAS,CUBLAS_DGEMVT_THREAD_COUNT>>>(params);
    } else {
        dgemvn_main<<<CUBLAS_DGEMVN_CTAS,CUBLAS_DGEMVN_THREAD_COUNT>>>(params);
    }
...
#define THREAD_CNT          (CUBLAS_DGEMVT_THREAD_COUNT)
#define TILEW_LOG           (5)
#define TILEW               (1 << TILEW_LOG)
#define TILEH_LOG           (5)
#define TILEH               (1 << TILEH_LOG)
#define TILE_ELEMS          ((TILEW) * (TILEH))
#define TJINC               ((CUBLAS_DGEMVT_CTAS) * (TILEW))
#define TIINC               (TILEH)
...
__shared__ double XX[TILEH];             /* cached portion of vector x */
__shared__ double AA[(TILEH+1)*TILEW];   /* cached portion of matrix A */
...
#define IINC                (CUBLAS_DGEMVN_CTAS * CUBLAS_DGEMVN_THREAD_COUNT)
#define JINC                (CUBLAS_DGEMVN_THREAD_COUNT * X_ELEMS_PER_THREAD)
#define XINC                (CUBLAS_DGEMVN_THREAD_COUNT)
...
__global__ void dgemvn_main (struct cublasDgemvParams parms) 
{
    ...
    for (i = 0; i < parms.m; i += IINC) {
        ...
        ii = i + blockIdx.x * CUBLAS_DGEMVN_THREAD_COUNT;            
